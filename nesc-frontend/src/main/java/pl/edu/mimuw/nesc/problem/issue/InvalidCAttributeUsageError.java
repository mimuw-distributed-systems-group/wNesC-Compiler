...
public final class InvalidCAttributeUsageError extends ErroneousIssue {
    ...
    public static InvalidCAttributeUsageError invalidScope() {
        final String description = "NesC attribute '@C()' cannot be used in this context; it must be specified in the implementation scope of a non-generic module";
        return new InvalidCAttributeUsageError(description);
    }

    public static InvalidCAttributeUsageError usageInGenericComponent() {
        final String description = "NesC attribute '@C()' cannot be used inside a generic component";
        return new InvalidCAttributeUsageError(description);
    }

    public static InvalidCAttributeUsageError usageForNescObject() {
        final String description = "NesC attribute '@C()' cannot be applied to a command, an event or a task";
        return new InvalidCAttributeUsageError(description);
    }

    public static InvalidCAttributeUsageError internalLinkage() {
        final String description = "NesC attribute '@C()' cannot be applied to an object with internal linkage";
        return new InvalidCAttributeUsageError(description);
    }

    public static InvalidCAttributeUsageError appliedToAttribute() {
        final String description = "NesC attribute '@C()' cannot be applied to an attribute definition";
        return new InvalidCAttributeUsageError(description);
    }
